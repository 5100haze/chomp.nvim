...
local SLAXML = require 'chomp.vendor.slaxml'
function SLAXML:dom(xml, opts)
  ...
  local builder = SLAXML:parser {
    ...
    comment = function(value) push(current.kids, { type = 'comment', name = '#comment', value = value, parent = rich and current or nil }) end,
    pi = function(name, value) push(current.kids, { type = 'pi', name = name, value = value, parent = rich and current or nil }) end,
  }
...
function SLAXML:xml(n, opts)
  ...
  function ser.comment(n, depth) table.insert(out, tab:rep(depth) .. '<!--' .. n.value .. '-->') end
